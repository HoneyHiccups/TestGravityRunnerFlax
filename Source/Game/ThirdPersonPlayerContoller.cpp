...
Float3 ThirdPersonPlayerContoller::GetFaceNormal() {
    ...
    if (Physics::RayCast(BodyHolder->GetPosition(), GravityDir, SurfaceGripper, 400.0f, 1, false)) {
        Float3 test = SurfaceGripper.Normal;
...
void ThirdPersonPlayerContoller::JustLanded() {
    openrezjumpcd = true;
    Float3 newfacenormal =  GetFaceNormal();
    if (newfacenormal != Vector3::Zero) {
        if (LevState > 0) {
            AugFalling(newfacenormal*-1.0f);
        }
    }
    LOG(Info, "new Face nromal {0}", newfacenormal);
}